...
class MIND_Corpus:
    ...
    def __init__(self, config: Config):
        ...
        with open(f'news2event{config.K}.pkl', 'rb') as fr:
            self.event_dict = pickle.load(fr)
        ...
        self.news_event_type = np.zeros([self.news_num, config.K], dtype=np.float32)
        ...
        for line in news_lines:
            ...
            self.news_event_type[index] = self.event_dict[news_ID]
            words = pat.findall(title.lower()) if config.tokenizer == 'MIND' else word_tokenize(title.lower())
