...
class Config:
    def parse_argument(self):
        ...
        # General config
        parser.add_argument('--mode', type=str, default='test', choices=['train', 'dev', 'test'], help='Mode')
        parser.add_argument('--news_encoder', type=str, default='Event_NAML_dsc2', help='News encoder')

        parser.add_argument('--user_encoder', type=str, default='Event_nrms_per_his',  help='User encoder')

        parser.add_argument('--score1', type=str, default='1', help='User encoder')
        parser.add_argument('--score2', type=str, default='1')
        parser.add_argument('--score3', type=str, default='0.6')
        parser.add_argument('--lossw', type=str, default='1')



        parser.add_argument('--dev_model_path', type=str, default='', help='Dev model path')
        parser.add_argument('--test_model_path', type=str, default='Best_model/best', help='Test model path')
        ...
        parser.add_argument('--seed', type=int, default=2020, help='Seed for random number generator')
        parser.add_argument('--config_file', type=str, default='', help='Config file path')
        # Dataset config
        parser.add_argument('--dataset', type=str, default='large', choices=['200k', 'small', 'large'], help='Dataset type')
        ...
        parser.add_argument('--max_title_length', type=int, default=20, help='Sentence truncate length for title')
        ...
        parser.add_argument('--num_attention_heads', type=int, default=15, help='Head number of multi-head self-attention')
        parser.add_argument('--num_filters', type=int, default=300, help='Head number of multi-head self-attention')
        parser.add_argument('--layers', type=int, default=6, help='Head number of multi-head self-attention')
        ...
        parser.add_argument('--dropout_probability', type=float, default=0.2, help='Dropout rate')
        parser.add_argument('--no_self_connection', default=False, action='store_true', help='Whether the graph contains self-connection')
        parser.add_argument('--query_vector_dim', default=200)
        ...
        parser.add_argument('--click_predictor', type=str, default='event', choices=['dot_product','dot_product2' 'mlp', 'sigmoid', 'FIM'], help='Click predictor')
        ...
        if self.dataset == 'small': # suggested configuration for MIND-small
            ...
            self.epoch = 16
            # self.epoch = 1
        elif self.dataset == '300k': # suggested configuration for MIND-200k
            self.dropout_rate = 0.2
            self.gcn_layer_num = 4
            self.epoch = 10
            # self.epoch = 1
        elif self.dataset == '500k': # suggested configuration for MIND-200k
            self.dropout_rate = 0.2
            self.gcn_layer_num = 4
            self.epoch = 10
        ...
        if self.news_encoder == 'MINS_ab': # suggested configuration for MIND-small
            self.dropout_rate = 0.2
            self.head_num=15
            self.category_embedding_dim=50
            self.subCategory_embedding_dim=50
            self.num_attention_heads=15
            self.max_abstract_length=50
            self.click_predictor='dot_product2'
            self.batch_size=64
            self.accum_steps=0
