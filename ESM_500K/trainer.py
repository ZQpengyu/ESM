...
class Trainer:
    def __init__(self, model: nn.Module, config: Config, mind_corpus: MIND_Corpus, run_index: int):
        ...
        self.loss = self.negative_log_softmax if config.click_predictor in ['dot_product', 'mlp', 'FIM', 'event'] else self.negative_log_sigmoid
        # self.loss = nn.CrossEntropyLoss() if config.click_predictor in ['dot_product', 'mlp', 'FIM',
        #                                                                     'event'] else self.negative_log_sigmoid
    ...
    def train(self):
        ...
        for e in tqdm(range(1, self.epoch + 1)):
            ...
            train_dataloader = DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True, #num_workers=self.batch_size // 16,
                                          num_workers=0,
                                          pin_memory=True)
            model.train()
            epoch_loss = 0
            ...
            #      user_event_type, news_event_type) in train_dataloader:
            for idx, (user_ID, user_category, user_subCategory, user_title_text, user_title_mask, user_title_entity,
                 user_content_text, user_content_mask, user_content_entity, user_history_mask,
                 news_category, news_subCategory, news_title_text, news_title_mask, news_title_entity,
                 news_content_text, news_content_mask, news_content_entity,
                 user_event_type, news_event_type) in enumerate(train_dataloader):
                ...
                user_event_type = user_event_type.cuda(non_blocking=True)
                news_event_type = news_event_type.cuda(non_blocking=True)
                ...
                #                news_category, news_subCategory, news_title_text, news_title_mask, news_title_entity, news_content_text, news_content_mask, news_content_entity, user_event_type, news_event_type) # [batch_size, 1 + negative_sample_num]
                logits = model(user_ID, user_category, user_subCategory, user_title_text, user_title_mask,
                               user_title_entity, user_content_text, user_content_mask, user_content_entity, user_history_mask,
                               news_category, news_subCategory, news_title_text, news_title_mask, news_title_entity,
                               news_content_text, news_content_mask, news_content_entity, user_event_type,
                               news_event_type)  # [batch_size, 1 + negative_sample_num]

                # loss = self.loss(logits, true)
                ...
                if model.user_encoder.auxiliary_loss is not None:
                    ...
                    loss += float(self.config.lossw) * user_encoder_auxiliary_loss
                # try:
                #     print(loss)
                # except:
                #     print(loss)
                ...
                if self.config.accum_steps > 0:
                    ...
                    loss.backward()
                    if (idx+1)%self.config.accum_steps == 0 or (idx+1) == len(train_dataloader):
                        self.optimizer.step()
...
def distributed_train(rank, model: nn.Module, config: Config, mind_corpus: MIND_Corpus, run_index: int):
    ...
    for e in tqdm(range(1, epoch + 1)):
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=batch_size, num_workers=1, pin_memory=True, sampler=train_sampler)
        model.train()
        epoch_loss = 0
        for (user_ID, user_category, user_subCategory, user_title_text, user_title_mask, user_title_entity,
            ...
            user_title_text = user_title_text.cuda(non_blocking=True)  # [batch_size, max_history_num, max_title_length]
            user_title_mask = user_title_mask.cuda(non_blocking=True)  # [batch_size, max_history_num, max_title_length]
            user_title_entity = user_title_entity.cuda(
                non_blocking=True)  # [batch_size, max_history_num, max_title_length]
            user_content_text = user_content_text.cuda(
                non_blocking=True)  # [batch_size, max_history_num, max_content_length]
            user_content_mask = user_content_mask.cuda(
                non_blocking=True)  # [batch_size, max_history_num, max_content_length]
            user_content_entity = user_content_entity.cuda(
                non_blocking=True)  # [batch_size, max_history_num, max_content_length]
            user_history_mask = user_history_mask.cuda(non_blocking=True)  # [batch_size, max_history_num]
            # user_history_graph = user_history_graph.cuda(non_blocking=True)                                                                                                 # [batch_size, max_history_num, max_history_num]
            # user_history_category_mask = user_history_category_mask.cuda(non_blocking=True)                                                                                 # [batch_size, category_num + 1]
            ...
            news_title_mask = news_title_mask.cuda(
                non_blocking=True)  # [batch_size, 1 + negative_sample_num, max_title_length]
            news_title_entity = news_title_entity.cuda(
                non_blocking=True)  # [batch_size, 1 + negative_sample_num, max_title_length]
            news_content_text = news_content_text.cuda(
                non_blocking=True)  # [batch_size, 1 + negative_sample_num, max_content_length]
            news_content_mask = news_content_mask.cuda(
                non_blocking=True)  # [batch_size, 1 + negative_sample_num, max_content_length]
            news_content_entity = news_content_entity.cuda(
                non_blocking=True)  # [batch_size, 1 + negative_sample_num, max_content_length]
            user_event_type = user_event_type.cuda(non_blocking=True)
            news_event_type = news_event_type.cuda(non_blocking=True)
            # logits = model(user_ID, user_category, user_subCategory, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, user_history_mask, user_history_graph, user_history_category_mask, user_history_category_indices, \
        ...
        # dev
        if rank == 0:
            auc, mrr, ndcg5, ndcg10 = compute_scores(model.module, mind_corpus, batch_size * 2, 'dev', dev_res_dir + '/' + model_name + '-' + str(e) + '.txt', config.dataset)
