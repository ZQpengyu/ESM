...
def generate_knowledge_entity_embedding(data_mode):
    assert data_mode in ['small', 'large', '500k']
    ...
    if data_mode in ['200k', 'small', '300k', '500k']:
...
def sampling_MIND_500k_dataset():
    sample_num = 500000
    # 1. randomly sample by users
    user_set = set()
    with open('../MIND-500k/download/train/behaviors.tsv', 'r', encoding='utf-8') as f:
        for line in f:
            impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
            user_set.add(user_ID)
    with open('../MIND-500k/download/dev/behaviors.tsv', 'r', encoding='utf-8') as f:
        for line in f:
            impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
            user_set.add(user_ID)
    ...
    with open('../MIND-500k/sample_users.json', 'w', encoding='utf-8') as f:
        json.dump(sample_user_list, f)
    sampled_user_set = set(sample_user_list)
    # 2. write sampled behavior file
    with open('../MIND-500k/download/train/behaviors.tsv', 'r', encoding='utf-8') as f:
        with open('../MIND-500k/train/behaviors.tsv', 'w', encoding='utf-8') as train_f:
            for line in f:
                impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
    ...
    cnt = 0
    with open('../MIND-500k/download/dev/behaviors.tsv', 'r', encoding='utf-8') as f:
        with open('../MIND-500k/dev/behaviors.tsv', 'w', encoding='utf-8') as dev_f:
            with open('../MIND-500k/test/behaviors.tsv', 'w', encoding='utf-8') as test_f:
                for line in f:
                    impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
                    if user_ID in sampled_user_set:
                        if cnt % 2 == 0:
    ...
    for mode in ['train', 'dev', 'test']:
        with open('../MIND-500k/%s/behaviors.tsv' % (mode), 'r', encoding='utf-8') as f:
            news_set = set()
            for line in f:
                impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
                if len(history) > 0:
                ...
                if len(impressions) > 0:
                    ...
                    for n in news:
                        news_set.add(n[:-2])
            with open('../MIND-500k/download/%s/news.tsv' % ('dev' if mode == 'test' else mode), 'r', encoding='utf-8') as _f:
                with open('../MIND-500k/%s/news.tsv' % mode, 'w', encoding='utf-8') as __f:
                    for line in _f:
...
def prepare_MIND_500k():
    download_extract_MIND_large('500k')
    sampling_MIND_500k_dataset()
    generate_knowledge_entity_embedding('500k')
