...
def download_extract_MIND_large(mode):
    assert mode in ['200k', 'large', '300k']
...
def generate_knowledge_entity_embedding(data_mode):
    assert data_mode in ['200k', '300k', 'small', 'large']
    ...
    if data_mode in ['200k', 'small', '300k']:
...
def sampling_MIND_300k_dataset():
    sample_num = 300000
    # 1. randomly sample by users
    user_set = set()
    with open('../MIND-300k/download/train/behaviors.tsv', 'r', encoding='utf-8') as f:
        for line in f:
            impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
            user_set.add(user_ID)
    with open('../MIND-300k/download/dev/behaviors.tsv', 'r', encoding='utf-8') as f:
        for line in f:
            impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
            user_set.add(user_ID)
    user_list = list(user_set)
    ...
    assert sample_num <= len(user_list), 'sample num must be less than or equal to 1000000'
    sample_user_list = random.sample(user_list, sample_num)
    with open('../MIND-300k/sample_users.json', 'w', encoding='utf-8') as f:
        json.dump(sample_user_list, f)
    sampled_user_set = set(sample_user_list)
    # 2. write sampled behavior file
    with open('../MIND-300k/download/train/behaviors.tsv', 'r', encoding='utf-8') as f:
        with open('../MIND-300k/train/behaviors.tsv', 'w', encoding='utf-8') as train_f:
            for line in f:
                impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
                if user_ID in sampled_user_set:
                    train_f.write(line)
    cnt = 0
    with open('../MIND-300k/download/dev/behaviors.tsv', 'r', encoding='utf-8') as f:
        with open('../MIND-300k/dev/behaviors.tsv', 'w', encoding='utf-8') as dev_f:
            with open('../MIND-300k/test/behaviors.tsv', 'w', encoding='utf-8') as test_f:
                for line in f:
                    impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
                    if user_ID in sampled_user_set:
                        if cnt % 2 == 0:
    ...
    # 3. write sampled news file
    for mode in ['train', 'dev', 'test']:
        with open('../MIND-300k/%s/behaviors.tsv' % (mode), 'r', encoding='utf-8') as f:
            news_set = set()
            for line in f:
                impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
                if len(history) > 0:
                ...
                if len(impressions) > 0:
                    ...
                    for n in news:
                        news_set.add(n[:-2])
            with open('../MIND-300k/download/%s/news.tsv' % ('dev' if mode == 'test' else mode), 'r', encoding='utf-8') as _f:
                with open('../MIND-300k/%s/news.tsv' % mode, 'w', encoding='utf-8') as __f:
                    for line in _f:
                        news_ID, category, subCategory, title, abstract, _, _, _ = line.split('\t')
                        if news_ID in news_set:
                            __f.write(line)
...
def prepare_MIND_300k():
    download_extract_MIND_large('300k')
    sampling_MIND_300k_dataset()
    generate_knowledge_entity_embedding('300k')
